...
export default function Settings() {
    ...
    useEffect(() => {
        // Intersection Observer to track scroll position and update active hash
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (id) {
                        // Soft update hash without jump
                        window.history.replaceState(null, '', `#${id}`);
                        // Dispatch hashchange for TopNav visibility logic
        ...
        const sectionIds = ['account', 'notifications', 'privacy', 'data', 'about'];
    ...
    return (
        <div className="flex h-screen bg-background text-foreground overflow-hidden">
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <main className="flex-1 overflow-y-auto scroll-smooth bg-background/50 relative scrollbar-thin scrollbar-thumb-white/10">
                    ...
                    {/* Continuous Settings Layout */}
                    <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 md:px-10 py-12 space-y-24 pb-32">

                        {/* Profile Section */}
                        <section id="account" className="scroll-mt-32">
                            <div className="glass-panel border border-border/30 rounded-[1.5rem] md:rounded-3xl p-5 sm:p-8 md:p-12 relative overflow-hidden">
                        ...
                        <section id="privacy" className="scroll-mt-32">
                            <div className="glass-panel border border-border/30 rounded-[1.5rem] md:rounded-3xl p-5 sm:p-8 md:p-12 relative overflow-hidden">
                                ...
                                <div className="bg-red-500/5 border border-red-500/20 p-5 md:p-8 rounded-[1.25rem] md:rounded-2xl mb-6 md:mb-8 max-w-4xl">
                                    <div className="flex items-start gap-3 md:gap-4 mb-5 md:mb-6">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[9px] md:text-[10px] text-muted-foreground uppercase leading-relaxed">
                                                Deleting your primary data vault is irreversible. Purges all history, biometrics, and index models.
                                            </p>
                                        </div>
                                    </div>
                                    <button className="w-full py-3.5 md:py-4 border border-red-500/30 rounded-lg md:rounded-xl text-red-500 font-extrabold text-[9px] md:text-[10px] tracking-[0.2em] md:tracking-[0.4em] uppercase hover:bg-red-500 hover:text-white transition-all">
                        ...
                        <section id="data" className="scroll-mt-32">
                            <div className="glass-panel border border-border/30 rounded-[1.5rem] md:rounded-3xl p-5 sm:p-8 md:p-12 relative overflow-hidden">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8 max-w-4xl">
                                    <div className="p-5 sm:p-6 md:p-8 bg-white/[0.02] border border-border/10 rounded-[1.25rem] md:rounded-2xl group hover:border-accent/20 transition-all flex flex-col justify-between">
                                        ...
                                        <p className="text-[9px] md:text-[10px] text-muted-foreground uppercase leading-relaxed mb-6 md:mb-8">SECURE TRANSACTION ARCHIVE.</p>
                                        <button className="w-full py-3.5 md:py-4 rounded-xl bg-accent/5 border border-accent/20 text-accent font-black text-[9px] tracking-widest uppercase hover:bg-accent hover:text-background transition-all">
                                            PREPARE EXPORT →
                                        </button>
                                    </div>
                                    <div className="p-5 sm:p-6 md:p-8 bg-white/[0.02] border border-border/10 rounded-[1.25rem] md:rounded-2xl group hover:border-accent/20 transition-all flex flex-col justify-between">
                                        <h4 className="text-[10px] md:text-xs font-black tracking-widest uppercase mb-3 md:mb-4 text-accent">TAX LEDGER (JSON)</h4>
                                        <p className="text-[9px] md:text-[10px] text-muted-foreground uppercase leading-relaxed mb-6 md:mb-8">AUDIT ENGINE COMPATIBILITY.</p>
                                        <button className="w-full py-3.5 md:py-4 rounded-xl bg-accent/5 border border-accent/20 text-accent font-black text-[9px] tracking-widest uppercase hover:bg-accent hover:text-background transition-all">
                                            PREPARE EXPORT →
                                        </button>
                                    </div>
                                ...
                                <div className="space-y-6 md:space-y-8 max-w-2xl">
                                    <div className="space-y-4">
                                        <p className="text-xs sm:text-sm font-black text-foreground uppercase tracking-widest leading-relaxed">
                                            Agentic, decentralized financial intelligence.
                                        </p>
                                        <p className="text-[10px] md:text-[11px] text-muted-foreground font-medium uppercase tracking-[0.1em] leading-loose opacity-70">
                                            Converging data modeling and cinematic immersion. distributed Oracle nodes.
                                        </p>
                                    </div>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-10 pt-6 border-t border-border/10">
                                        <div><span className="text-accent text-[8px] font-black tracking-widest block mb-1 uppercase opacity-60">BUILD</span><span className="text-foreground text-[10px] font-black tracking-widest uppercase">NODE_V26_RELEASE</span></div>
                                        <div><span className="text-accent text-[8px] font-black tracking-widest block mb-1 uppercase opacity-60">SECURITY</span><span className="text-foreground text-[10px] font-black tracking-widest uppercase">AES-256 QUANTUM-SAFE</span></div>
                                    </div>
                                </div>
                            </div>
                        </section>
                    ...
                    {/* Action Buttons */}
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 pb-12 flex flex-col-reverse md:flex-row justify-end items-center gap-4 md:gap-10 relative z-20">
                        <button
